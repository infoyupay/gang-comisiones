 ...
 *     <li>Designed for usage in test environments, particularly for non-nullity validation scenarios.</li>
 * </ul>
 *
 * @param <T>         the type of the object being validated.
 * @param description the description for the assertion failure message.
 * @param extracting  the functions to extract fields values whose non-nullity must be asserted.
 * @author InfoYupay SACS
 * @version 1.0
 */
public record FieldsMustBeNonNullAssertion<T>(@NotNull String description,
                                              @NotNull Function<? super T, ?> @NotNull ... extracting) {

    /**
     * Constructs an instance of {@code FieldsMustBeNonNullAssertion}, a utility for performing
     ...
     *     <li>Designed for flexible and reusable non-null verification logic in test cases.</li>
     * </ul>
     *
     * @param description the description for the assertion failure message.
    ...
    @Contract(pure = true)
    @SafeVarargs
    public FieldsMustBeNonNullAssertion {
    }
     ...
     * are non-null. The assertions will be softly applied, and any failures will be collected
    ...
    public static <T> @NotNull Consumer<SoftAssertions> softly(
        ...
        return softly -> {
            softly.assertThat(value)
                    ...
                    .isNotNull();
            new FieldsMustBeNonNullAssertion<>(description, extracting).softlyAssertNonNull(softly, value);
        };
    }

    /**
     * Asserts that all values extracted using the provided functions are non-null,
     ...
     * of a given value are non-null during testing.
     * <br/>
     ...
     * </ul>
     *
     * @param softly the {@link SoftAssertions} instance to collect the non-null assertions.
     *               <ul>
     *                   <li>Must not be {@code null}.</li>
     *               </ul>
     * @param value  the object whose fields or properties are accessed and tested for non-null.
     *               <ul>
     *                   <li>Must not be {@code null}.</li>
     *               </ul>
     */
    public void softlyAssertNonNull(SoftAssertions softly, T value) {
        Stream.of(extracting)
                .map(f -> f.apply(value))
                .forEach(obj -> softly
                        .assertThat(obj)
                        .as("%s found null", description)
                        .isNotNull());
    }
}
